...
Точные REST-схемы, коды состояний и ошибок, модель возможностей сканера и политика файлового обмена описаны в [`docs/api.md`](docs/api.md). Полная runtime/package-time конфигурация, defaults, provider priority и platform-specific paths описаны в [`docs/configuration.md`](docs/configuration.md), а machine-readable JSON projection — в [`docs/appsettings.schema.json`](docs/appsettings.schema.json).
