...
Историческая цепочка SDK сохраняется в истории Git, но старые бинарные поколения не обязаны одновременно находиться в текущем checkout:
...
В текущем `vendor/nuget` хранятся только текущие repository-owned SDK и Win32 worker, необходимые текущему restore/build. Старые поколения никогда не пересобираются и не перезаписываются: для воспроизведения старого состояния используется checkout соответствующего commit или tag из истории Git.
...
6. только после успешной сборки и канонизации обоих текущих пакетов удаляет из текущего `vendor/nuget` предыдущие поколения repository-owned NAPS2 packages, не изменяя историю Git.
...
Исторический worker `.1` имеет SHA-256 `18993608e478661100df88f2ea80fcd87c88719d05b0b8f6230da18b582ed691` и остаётся воспроизводимым через соответствующий commit/tag в истории Git; его бинарник не хранится в текущем HEAD.
