...
Без environment-specific конфигурации безопасный пакет содержит пустую карту `WebAssistant:FileSystem`, поэтому файловый обмен не получает автоматический физический корень. Именованные корни задаются явно, например:
