...
Все операции только для чтения используют `GET`, все изменения — `POST`. Файловых маршрутов `PUT` и `DELETE` нет. Отсутствуют универсальный маршрут файловой системы, серверная сессия `cd` и точка API для произвольного пути хоста.
...
У каждой панели собственные логический корень, текущий каталог, сортировка, вертикальная прокрутка, wildcard и набор выбранных файлов. Каталог открывается щелчком; строка `..` поднимает на уровень вверх. Поля wildcard по умолчанию содержат `*.*`; каталоги остаются видимыми независимо от фильтра.
...
В заголовках панелей доступны пакетные действия `Действия [→]` и `Действия [←]`. Они отправляют один `POST /v1/filesystem/move` для всех выбранных обычных файлов и отключены при пустом selection или одинаковом source/destination. Стрелка отдельного файла использует тот же batch API с массивом из одного имени. Перемещение каталога использует `/directory/move`, а переименование — `/rename`; переименование через `move` не моделируется.
