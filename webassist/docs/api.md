...
Все операции только для чтения используют `GET`, все изменения — `POST`. Файловых маршрутов `PUT` и `DELETE` нет. Маршруты с JSON-телом — `POST /v1/filesystem/directory`, `POST /v1/filesystem/move`, `POST /v1/filesystem/directory/move` и `POST /v1/filesystem/rename` — требуют `Content-Type: application/json`; отсутствующий или иной тип содержимого возвращает `400 filesystem_path_invalid`. Загрузка через `POST /v1/filesystem/file` передаёт непрозрачные байты и JSON не использует. Отсутствуют универсальный маршрут файловой системы, серверная сессия `cd` и точка API для произвольного пути хоста.

### Получение списка каталога и wildcard

`GET /v1/filesystem/list?path=<logical-directory>&wildcard=<masks>&cursor=<opaque>&limit=<n>`

`path` — логический путь каталога. Для логического корня `archive` верхний уровень задаётся как `archive/`. Пустой `path` недопустим. `limit` по умолчанию равен `200`, максимальное значение — `1000`.

`wildcard` необязателен. При отсутствии возвращаются все допустимые записи по прежней семантике. Если параметр передан, но после нормализации не содержит масок, возвращается `400 filesystem_path_invalid`.
...
- маска применяется только к имени записи и не может содержать `/` или `\\`;
- сопоставление регистронезависимое на всех ОС;
- каталоги возвращаются независимо от wildcard;
- фильтр применяется только к непосредственным обычным файлам текущего каталога, без рекурсии;
- фильтрация выполняется до публичной пагинации;
- `*.*` означает все обычные файлы, включая имена без точки, например `README`;
- `cursor` связан с логическим корнем, каталогом и каноническим effective wildcard; повторное использование cursor с другим фильтром отклоняется.
...
Архив содержит только непосредственные обычные файлы текущего каталога, совпадающие с теми же wildcard-правилами, что и `list`. Каталоги и вложенные файлы не включаются, рекурсии нет. Имя ZIP-entry — только фактическое basename файла; родительские и физические пути в архив не попадают. Ноль совпадений — корректный пустой ZIP с `200`.

ZIP формируется потоково: WebAssistant не буферизует целиком произвольный файл или весь архив в памяти. До начала ответа выполняется максимально возможная проверка выбранных записей. Если после начала HTTP-ответа чтение файла или нативная проверка завершаются ошибкой, сервер прерывает поток вместо выдачи ложного успешно завершённого ZIP.

### Поиск точных имён

`GET /v1/filesystem/find?path=<logical-directory>&name=a.xml&name=b.xml`

Параметр `name` повторяемый, от 1 до 100 значений. Дубликаты отклоняются с `400`. Каждое имя — ровно один элемент каталога без разделителей пути, `.` и `..`.

Сравнение выполняется строго по фактическому имени записи с `StringComparison.Ordinal`, то есть с учётом регистра на всех ОС, включая регистронезависимые файловые системы. Возвращаются только существующие обычные неограниченные файлы непосредственного каталога. Каталоги, wildcard и рекурсия не участвуют.

Ответ сохраняет порядок имён из запроса; отсутствующие имена пропускаются:
...
Ноль найденных файлов — успешный `200` с пустым массивом.
...
`sourcePath` и `destinationPath` — логические каталоги одного корня. `fileNames` обязателен и содержит от 1 до 1000 уникальных имён. Каждое имя — один элемент каталога без разделителей пути, абсолютного пути, `.` и `..`.

Операция принимает только обычные файлы. Wildcard и рекурсия не используются. Исходный и целевой каталоги не могут совпадать. Межкорневое перемещение запрещено.
...
Каждый фактически выполненный перенос — атомарный `no-replace` без схемы copy-delete. Отсутствующий source и уже существующий destination для конкретного имени пропускаются. Ответ содержит только действительно перемещённые имена в порядке запроса:
...
Перемещается один каталог. `destinationPath` задаёт только родительский каталог назначения. Итоговое имя всегда равно фактическому basename `sourcePath`; параметра нового имени нет. Логический root object перемещать нельзя. Источник и назначение должны находиться в одном логическом корне, а назначение не может быть самим источником или его потомком. Существующее назначение даёт `409 destination_exists`.
...
Каждая операция разрешается внутри одного выбранного rooted authority. Недоступность одного корня не меняет область полномочий другого и не отключает сканирование или `GET /v1/health`.
...
Выбранный логический корень общий для обеих панелей. При переключении логического корня LEFT и RIGHT одновременно переходят в его верхний уровень и сбрасывают своё выделение. У каждой панели независимо хранятся текущий каталог, сортировка, вертикальная прокрутка, wildcard и набор выбранных файлов. Каталог открывается щелчком; строка `..` поднимает на уровень вверх. Поля wildcard по умолчанию содержат `*.*`; каталоги остаются видимыми независимо от фильтра.
...
В заголовках панелей доступны пакетные действия `Действия →` и `Действия ←`. Они отправляют один `POST /v1/filesystem/move` для всех выбранных обычных файлов и отключены при пустом выделении или одинаковом исходном и целевом каталоге. Стрелка отдельного файла использует тот же пакетный API с массивом из одного имени. Перемещение каталога использует `/directory/move`, а переименование — `/rename`; переименование через `move` не моделируется.

Загрузка выполняется за один пользовательский шаг: после выбора файла отправка начинается сразу. ZIP скачивает текущий каталог с текущим wildcard. Поиск принимает набор точных имён, вызывает `/find`, помечает найденные строки и добавляет их в selection; ноль найденных файлов считается успешным результатом без смены каталога.
