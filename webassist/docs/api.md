...
До открытия физического корня WebAssistant синтаксически разбирает логический путь и выбирает его первый сегмент. Неизвестный логический корень не получает fallback на другой корень.
...
Каждый логический корень имеет отдельный нативный контекст полномочий. Недоступность одного корня не изменяет authority другого и не отключает сканирование или `GET /v1/health`.
...
В верхней части страницы расположены кнопки доступных логических корней. После выбора корня LEFT и RIGHT работают независимо: у каждой панели собственные текущий каталог, хлебные крошки, сортировка и обновление. Каталог открывается по щелчку, строка `..` поднимает на уровень вверх, а на корне остаётся внутри выбранного логического корня. Кнопки `→` и `←` перемещают объект только между каталогами того же активного корня и отключены для одинаковых каталогов.
...
`filesystem_not_configured` означает отсутствующую или пустую карту `WebAssistant:FileSystem`. `filesystem_configuration_invalid` означает некорректность самой карты, например запрещённое имя или case-only collision. `filesystem_root_not_found` означает синтаксически корректное, но неизвестное логическое имя. `filesystem_root_unavailable` относится только к выбранному настроенному корню, который сейчас невозможно безопасно открыть или использовать. `filesystem_path_invalid` покрывает некорректный логический путь, неверный cursor и межкорневое перемещение.
