...
```text
archive/incoming/2026/document.pdf
```
...
Ответ содержит только логические имена. Физические пути не публикуются. Недоступный физический каталог остаётся логически настроенным корнем и не удаляет другие корни из конфигурации.

### Маршруты

Публичная поверхность файлового обмена:

```text
GET    /v1/filesystem/roots
GET    /v1/filesystem/list?path=<logical-path>&cursor=<opaque>&limit=<n>
GET    /v1/filesystem/file?path=<logical-path>
PUT    /v1/filesystem/file?path=<logical-path>
DELETE /v1/filesystem/file?path=<logical-path>
POST   /v1/filesystem/directory
DELETE /v1/filesystem/directory?path=<logical-path>
...
`path` — логический путь каталога. Для логического корня `archive` его верхний уровень задаётся как `archive/`. Пустой `path` недопустим, потому что он не выбирает логический корень. `limit` по умолчанию равен `200`, максимальное значение — `1000`. `cursor` непрозрачен, привязан к конкретному логическому корню и действителен только для того каталога, для которого был выдан.
...
```json
{
  ...
  "destinationPath": "archive/processed/a.bin"
}
```

Источник и назначение обязаны принадлежать одному и тому же логическому корню. Межкорневое перемещение, например из `archive/...` в `import/...`, возвращает `400 filesystem_path_invalid` ещё до открытия области полномочий корня назначения. Резервный вариант «копировать и удалить» не используется.

Внутри одного логического корня перемещение выполняется как атомарное переименование без замены назначения. Успех: `204 No Content`.

### Удаление

`DELETE /v1/filesystem/file?path=<logical-path>` удаляет обычный файл.

`DELETE /v1/filesystem/directory?path=<logical-path>` удаляет только пустой каталог. Рекурсивное удаление через API отсутствует.

Успех: `204 No Content`.

### Политика путей и ссылок

До открытия физического корня WebAssistant синтаксически разбирает логический путь и выбирает его первый сегмент. Неизвестный логический корень не приводит к автоматическому выбору другого корня.
...
Каждый логический корень имеет отдельный нативный контекст полномочий. Недоступность одного корня не изменяет область полномочий другого и не отключает сканирование или `GET /v1/health`.
...
Загрузка с заблокированным расширением и перемещение из или в такое имя возвращают `422 blocked_file_type`. Если ограниченный файл создан внешней программой напрямую внутри одного из настроенных физических корней, список может показать его с `restrictionCode=active_extension`, но WebAssistant не отдаёт его через скачивание и не переименовывает. Удаление такой записи каталога разрешено для очистки.
...
`/filesystem.html` — принадлежащий репозиторию двухпанельный клиент того же публичного API файлового обмена. У страницы нет закрытых или тестовых привилегий: все операции выполняются через перечисленные выше маршруты `/v1/filesystem/*`.

В верхней части страницы расположены кнопки доступных логических корней. После выбора корня левая и правая панели работают независимо: у каждой панели собственные текущий каталог, хлебные крошки, сортировка и обновление. Каталог открывается по щелчку, строка `..` поднимает на уровень вверх, а на корне остаётся внутри выбранного логического корня. Кнопки `→` и `←` перемещают объект только между каталогами того же активного корня и отключены для одинаковых каталогов.

Загрузка выполняется за один пользовательский шаг: нажатие кнопки открывает выбор файла, после выбора загрузка начинается сразу. Щелчок по файлу запускает скачивание. Каталоги и файлы визуально различаются. Встроенного просмотра или исполнения содержимого нет.

Переключение активного логического корня сбрасывает обе панели в корень нового пространства; пути предыдущего корня не переносятся. Внешние изменения файловой системы становятся видимыми после `Обновить`.
...
`filesystem_not_configured` означает отсутствующую или пустую карту `WebAssistant:FileSystem`. `filesystem_configuration_invalid` означает некорректность самой карты, например запрещённое имя или конфликт имён, различающихся только регистром. `filesystem_root_not_found` означает синтаксически корректное, но неизвестное логическое имя. `filesystem_root_unavailable` относится только к выбранному настроенному корню, который сейчас невозможно безопасно открыть или использовать. `filesystem_path_invalid` покрывает некорректный логический путь, некорректное значение `cursor` и межкорневое перемещение.

Ответ об ошибке и технические журналы не раскрывают `physicalRootPath`. В журнале используются только `logicalRoot` и относительный путь внутри него.