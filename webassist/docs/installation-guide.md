...
Конфигурация пакета выбирается **во время сборки**. После формирования установщика или пакета файл `appsettings.json` является частью payload пакета. Установщик не создаёт, не заменяет и не исправляет упакованную конфигурацию. Полная структура `appsettings.json`, defaults, provider priority и platform-specific пути описаны в [`configuration.md`](configuration.md).
