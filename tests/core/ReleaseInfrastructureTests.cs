...
public sealed class ReleaseInfrastructureTests
{
    ...
    private static readonly string Version = ReadRepositoryVersion();
    ...
    public void AcceptedMainResolver_AcceptsExactMergedPrWithGreenOwnedWorkflowsAndMonotonicVersion()
    {
        ...
        Assert.Equal("feature/release-fixture", json.RootElement.GetProperty("prHeadRef").GetString());
    ...
    public void DraftStaging_CreatesUnpublishedDraftAndStagesSixExactAssets()
    {
        ...
        Assert.Contains($"release upload v{Version}", log, StringComparison.Ordinal);
    ...
    public void Finalizer_PublishesValidCompleteDraftWithoutRebuild()
    {
        ...
        Assert.Equal(0, result.ExitCode);
        var log = File.ReadAllText(github.LogPath);
        Assert.Contains($"release download v{Version}", log, StringComparison.Ordinal);
        Assert.Contains($"release upload v{Version}", log, StringComparison.Ordinal);
        Assert.Contains("WebAssistant-Installation-Guide.pdf", log, StringComparison.Ordinal);
        Assert.Contains($"release edit v{Version}", log, StringComparison.Ordinal);
    ...
    private static string ReadRepositoryVersion() => File.ReadAllText(Path.Combine(FindRepositoryRoot(), "webassist", "VERSION")).Trim();
    ...
    private sealed class ResolverFixture : IDisposable
    {
        ...
        public static ResolverFixture Create(string? mainSha = null, bool includePr = true, string repoGuardConclusion = "success", string ciRequiredConclusion = "success", string repoGuardJobConclusion = "success", string? sourceVersion = null)
        {
            ...
            WriteJson(Path.Combine(data, "pulls.json"), includePr ? new object[] { new { number = 17, created_at = "2026-09-09T19:00:00Z", merged_at = "2026-09-09T19:40:05Z", merge_commit_sha = SourceSha, head = new { sha = PrHeadSha, @ref = "feature/release-fixture" }, @base = new { @ref = "main", sha = BaseSha } } } : Array.Empty<object>());
            WriteJson(Path.Combine(data, "runs.json"), new { workflow_runs = new object[] { new { id = 101, path = ".github/workflows/ci.yml", @event = "pull_request", status = "completed", conclusion = "success", head_sha = PrHeadSha, head_branch = "feature/release-fixture", created_at = "2026-09-09T19:30:00Z", pull_requests = Array.Empty<object>() }, new { id = 102, path = ".github/workflows/repo-guard.yml", @event = "pull_request", status = "completed", conclusion = repoGuardConclusion, head_sha = PrHeadSha, head_branch = "feature/release-fixture", created_at = "2026-09-09T19:30:00Z", pull_requests = Array.Empty<object>() } } });
...
""");
    ...
    private sealed class ReleaseGhFixture : IDisposable
    {
        ...
        public static ReleaseGhFixture Create(string state, InstallerFixture? installers = null, FinalEvidenceFixture? evidence = null, string? mainSha = null)
        {
            ...
            WriteJson(Path.Combine(data, "draft-release.json"), new object[] { new { id = 77, tag_name = $"v{Version}", target_commitish = SourceSha, draft = true, prerelease = false, body } });
            WriteJson(Path.Combine(data, "published-release.json"), new object[] { new { id = 77, tag_name = $"v{Version}", target_commitish = SourceSha, draft = false, prerelease = false, body } });
...
if [[ "${1:-}" == "api" && "${2:-}" == "repos/test/repo/git/ref/tags/v$FAKE_VERSION" ]]; then
...
if [[ "${1:-}" == "api" && "${2:-}" == "--method" && "${3:-}" == "POST" ]]; then
  endpoint="${4:-}"
  case "$endpoint" in
    repos/test/repo/git/refs) touch "$FAKE_GH_ROOT/tag-created"; printf '{"ref":"refs/tags/v%s"}\n' "$FAKE_VERSION" ;;
    repos/test/repo/releases) touch "$FAKE_GH_ROOT/draft-created"; printf '{"id":77,"draft":true,"tag_name":"v%s"}\n' "$FAKE_VERSION" ;;
