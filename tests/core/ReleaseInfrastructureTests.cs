...
public sealed class ReleaseInfrastructureTests
{
    ...
    {
        using var fixture = InstallerFixture.Create();
        using var github = ReleaseGhFixture.Create(state: "draft-conflict");
        var result = RunReleaseScript(
            "stage-draft-release.sh",
            [fixture.Root, Version, SourceSha, "555"],
            github.Environment,
            github.BinDirectory);
        Assert.NotEqual(0, result.ExitCode);
    ...
    {
        using var fixture = InstallerFixture.Create();
        using var github = ReleaseGhFixture.Create(state: "absent");
        var result = RunReleaseScript(
            "stage-draft-release.sh",
            [fixture.Root, Version, SourceSha, "555"],
            github.Environment,
    ...
    {
        using var github = ReleaseGhFixture.Create(state: "complete-draft", mainSha: new string('d', 40));
        var root = FindRepositoryRoot();
        ...
        var result = RunReleaseScript(
            "finalize-release.sh",
            ["--source-sha", SourceSha, "--evidence", evidence, "--pdf", pdf],
            github.Environment,
            github.BinDirectory);
        Assert.NotEqual(0, result.ExitCode);
        Assert.Contains("current main", result.Error, StringComparison.OrdinalIgnoreCase);
...
""");
    ...
    {
        private InstallerFixture(string root, string windowsArtifact) { Root = root; WindowsArtifact = windowsArtifact; }
        public string Root { get; }
        ...
        public static InstallerFixture Create()
        {
            ...
            WriteSidecars(windows, "win-x64"); WriteSidecars(linux, "linux-x64");
            return new InstallerFixture(root, windows);
        }
        private static void WriteSidecars(string artifact, string rid)
        {
            var name = Path.GetFileName(artifact); var digest = Sha256(artifact);
    ...
    private sealed class ReleaseGhFixture : IDisposable
    {
        ...
        public static ReleaseGhFixture Create(string state, string? mainSha = null)
        {
            var root = Directory.CreateTempSubdirectory("webassistant-release-gh-").FullName;
            var bin = Directory.CreateDirectory(Path.Combine(root, "bin")).FullName;
            var log = Path.Combine(root, "gh.log"); File.WriteAllText(log, string.Empty);
            var fakeGh = Path.Combine(bin, "gh");
            File.WriteAllText(fakeGh, """
#!/usr/bin/env bash
set -euo pipefail
...
if [[ "${1:-}" == "api" && "${2:-}" == "repos/test/repo/releases?per_page=100" ]]; then
  case "$FAKE_RELEASE_STATE" in
    absent) printf '[]\n' ;;
    draft-conflict|complete-draft) printf '[{"id":77,"tag_name":"v0.3.19","target_commitish":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","draft":true,"prerelease":false,"body":"webassistant-release-candidate/v1"}]\n' ;;
  esac
  exit 0
fi
if [[ "${1:-}" == "api" && "${2:-}" == "repos/test/repo/git/ref/tags/v0.3.19" ]]; then
  [[ "$FAKE_RELEASE_STATE" == absent ]] && exit 1
...
if [[ "${1:-}" == "api" && "${2:-}" == "repos/test/repo/releases/77/assets?per_page=100" ]]; then
  if [[ "$FAKE_RELEASE_STATE" == draft-conflict ]]; then
    printf '[{"name":"WebAssistant-win-x64-0.3.19.exe","digest":"sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"}]\n'
  else
    printf '[]\n'
  fi
  exit 0
...
if [[ "${1:-}" == "api" && "${2:-}" == "--method" && "${3:-}" == "POST" ]]; then
  printf '{"id":77,"draft":true,"tag_name":"v0.3.19"}\n'; exit 0
fi
...
""");
            SetExecutable(fakeGh);
            return new ReleaseGhFixture(root, bin, log, new Dictionary<string, string> { ["GH_REPO"] = "test/repo", ["FAKE_GH_LOG"] = log, ["FAKE_RELEASE_STATE"] = state, ["FAKE_MAIN_SHA"] = mainSha ?? SourceSha });
        }
        public void Dispose() => Directory.Delete(Root, recursive: true);
    }
