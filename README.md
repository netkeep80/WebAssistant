...
Конфигурация времени выполнения формируется стандартными configuration providers ASP.NET Core; package `appsettings.json`, его структура, defaults, provider priority и platform-specific пути описаны в [`webassist/docs/configuration.md`](webassist/docs/configuration.md). CORS по умолчанию выключен и включается только явным списком разрешённых origins. Текущая реализация кандидата v0.3 предоставляет multi-root файловый обмен: каждый путь начинается с настроенного logical root, публичная filesystem API использует только `GET` и `POST`, а визуальный клиент доступен по `/filesystem.html`. Текущей принятой парой contract/conformance является v0.2.1; v0.3 остаётся кандидатом.

Продуктовая документация: [`webassist/README.md`](webassist/README.md), [`webassist/docs/api.md`](webassist/docs/api.md), [`webassist/docs/configuration.md`](webassist/docs/configuration.md) и machine-readable [`webassist/docs/appsettings.schema.json`](webassist/docs/appsettings.schema.json). Карта всех актуальных документов и правила обязательного full-document audit находятся в [`docs/documentation-governance.md`](docs/documentation-governance.md).
